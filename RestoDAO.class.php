...
class RestoDAO {
    ...
    public static function getAllByType($lesTypes): array{
        $lesObjets = array();
        try {
                    // Initialisez la requête SQL en dehors de la boucle
                    $requete = "SELECT * FROM resto r 
                    INNER JOIN tc_li tl ON r.idR = tl.idRe 
                    INNER JOIN typecuisine t ON tl.idTc = t.id";

                    // Initialisez le tableau de conditions
                    $conditions = [];

                    // Boucle à travers les types de cuisine
                    foreach($lesTypes as $unType) {
                    // Ajoutez chaque condition à un tableau
                        $conditions[] = "t.libelle LIKE '%" . $unType . "%'";
                    }

                    // Si des conditions existent, ajoutez-les à la requête
                    if (!empty($conditions)) {
                    // Utilisez la fonction implode() pour joindre les conditions avec "OR"
                        $requete .= " WHERE " . implode(" OR ", $conditions) . ";";
                    } else {
                        // Sinon, la requête n'a pas de condition WHERE
                        $requete .= ";";
                    }
            ...
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::getAllByType : <br/>" . $e->getMessage());
        }
        return $lesObjets;
    }
